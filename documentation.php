...
/**
 * Documentation of Cygnite Framework
 *
 * @author SANJOY
 */
 ...
 * $this->cygnite->where('username','Sanjay');
 * $this->cygnite->order_by('id','DESC');
 * $this->cygnite->limit(3);
 * $this->cygnite->select('username,country');
 * $resultArray = $this->cygnite->fetch_all('user_details');
 *
 */
 ...
 *Example usage :$this->cygnite->where('username','Sanjoy Dey'); OR
 *$where = array(
                            'username =' => 'Sanjoy Dey',
                            'id >'=> '4'
                    );
 ...
 * $this->cygnite->order_by('id');               ( i.e )  ORDER BY id ASC
 * $this->cygnite->order_by('id','DESC');  ( i.e )  ORDER BY id DESC
 */
 ...
 *Example usage :
 * $this->cygnite->limit(3);     i.e LIMT 0,3
 *$this->cygnite->limit(1,3);  i.e LIMIT 1,3
 */
 ...
 * Example usage:
 *$this->cygnite->select('username,country'); (i.e) SELECT username,country FROM tablename;
 * OR
 * $this->cygnite->select('all');   (i.e) SELECT * FROM tablename; to select all fields of the table *
 */
 ...
 *Example usage:  $this->db_name->fetch_all('guestbook');
 */
 ...
 *Example usage: $this->cygnite->debug_query();
 *
 */
