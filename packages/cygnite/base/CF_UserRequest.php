    ...
    public function handle()
    {
        if($this->router->current_uri() =='/' ||  $this->router->current_uri() == '/'.self::$index_page):
            if($this->default['controller'] !=''){
                include str_replace('/','',APPPATH).DS.'controllers'.DS.$this->default['controller'].EXT;
                $defaultController = ucfirst($this->default['controller']).ucfirst(str_replace('/','',APPPATH)).'Controller';
            ...
            $this->router->get('/',call_user_func_array(array(new $defaultController,$defaultAction),$param_arr = array()));
                   ...
                   $matched_url = $this->matches($routeConfig);
                    // Custom 404 Handler
                    Cygnite::loader()->request('Router')->set404(function() {
                            ...
                            if(!is_null($matched_url)) :
                                    $requesturi = preg_split('/[\.\ ]/', $matched_url['controlpath']);
                                     $controller = ucfirst($requesturi[0]).ucfirst(str_replace('/','',APPPATH)).'Controller';
                                     $action = 'action_'.strtolower($requesturi[1]);
                                     include str_replace('/','',APPPATH).DS.'controllers'.DS.$requesturi[0].EXT;
                                     call_user_func_array(array(new $controller,$action), (array)$matched_url['params'][0]);
                             else:
                                               $controller = ucfirst($exp[1]).ucfirst(str_replace('/','',APPPATH)).'Controller';  
                                               $includepath = str_replace('/','',APPPATH).DS.'controllers'.DS.strtolower($exp[1]).EXT;
                                              $callee = debug_backtrace();
                                              if(is_readable($includepath)):
                                                    include str_replace('/','',APPPATH).DS.'controllers'.DS.strtolower($exp[1]).EXT;
                                              else:
                                                        GHelper::trace();
