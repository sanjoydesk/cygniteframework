    ...
    protected function connect()
    {
        ...
        try{ 
            foreach ($config as $key => $value) {
                if(class_exists('PDO') && !is_null($value['database']) && !is_null($value['hostname'])){  
                    $this->conn[$value['database']] = new PDO("mysql:host=".$value['hostname'].";dbname=".$value['database'], $value['username'],$value['password']);
                } 
                
                $this->__set($value['database'],$this->conn[$value['database']]);
                $this->conn[$value['database']]->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);                
            }
...
class Generic_SQL_Builder extends DBConnection
{   
    private $pdo;
    public $connobj,$connection;
...
class Test 
{
    public $arr = array(); 
    private $db;
    private $instance,$selectinstance,$type,$connect;


    public function __construct($dbkey)
    {       
        $this->connect = $dbkey;
        $this->db = new Generic_SQL_Builder($this->connect);
...
class AUser extends Test
{
    ...
    {   
        ...
        $this->status = '1';
        $this->delete_tips = '0';
       // $this->save('vod_tips',array('tips_id'=> 5));
