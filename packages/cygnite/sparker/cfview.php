     ...
     */
     ...
     protected function createsections(array $sections)
     {
           foreach($sections as $key=>$value):
               $this->template[$key] = str_replace(':', '/', $value);
           endforeach;
     ...
     public function setlayout($layout, array $params)
     {
          ...
          $this->requestedcontroller = strtolower(str_replace('Apps\\Controllers\\','',$trace[1]['class']));
          
          $this->layoutparams = $params;
          
          if(is_readable($this->views.rtrim(str_replace(array('.','/',':'),DS,$layout).'.layout').EXT)):
                ob_start();
                 $this->view_path = $this->views.rtrim(str_replace(array('.','/',':'),DS,$layout).'.layout').EXT;
          endif;
          $this->loadview();
     ...
     public function with($arr_params)
     {
          if(is_array($arr_params))
                    $this->params = (array) $arr_params;
          $this->loadview();
     }
